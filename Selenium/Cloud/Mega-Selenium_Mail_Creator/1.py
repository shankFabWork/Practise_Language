...
def checkMails():
    ...
    if length == 0:
        ...
        print(file_address)
        myString = open(file_address).read()
        mail_content = re.search("(?P<url>https?://[^\s]+)", myString).group("url")
        # final_data = f.readline()
        # final_data = final_data.split(":")[1]
        # mail_content=final_data[1::]
        print(mail_content)

        driver.get(mail_content)
...
# Mail
mail_address = driver.find_element_by_xpath('//*[@id="register-email-registerpage2"]')
mail_address.click()
print(mail_name)
mail_address.send_keys(mail_name)
# First Password
...
print(mail_name+" is made successfully")
