#include <iostream>
#include <vector>

using namespace std;

class Node
{
    public:
    int data;
    Node *next;
    Node *prev;


    Node()
    {
        next = NULL;
        prev = NULL;
    }

    Node(int x)
    {
        data = x;
        next = NULL;
        prev = NULL;
    }
};

Node* Create_Linked_List()
{
    Node *temp = new Node();   
    Node *start = new Node();
    temp = NULL;   
    start = NULL;

    char ch;

    do
    {
        if(start == NULL)
        {
            Node* New = new Node();
            cout<<endl<<"please insert your node ";
            cin>>New->data;
            start = New;
            temp = New;
            New->prev = NULL;

        }
        else
        {
            Node* New = new Node();
            cout<<endl<<"please insert your node ";
            cin>>New->data;
            temp->next = New;
            New->prev = temp;
            temp=New;
        }
        cout<<endl<<"Do u want insert Node again ";
        cin>>ch;
    }while(ch == 'y' || ch == 'Y');


    temp->next = start;
    return start;
}


void Display_Linked_List(Node *start)
{
    Node* temp = start;
    cout<<endl<<"Nodes are ";

    do
    {
        cout<<temp->data<<" ";
        temp=temp->next;
    }while(temp != start);

    cout<<endl;
}

Node* Insert_From_start(Node* start)
{
    Display_Linked_List(start);

    int elem;
    cout<<endl<<"enter element to insert at the start -> ";
    cin>>elem;

    Node* temp=start;
    do
    {
        temp=temp->next;
    }while(temp->next != start);

    Node* New = new Node(elem);
    New->next = start;
    New->prev=NULL;
    start->prev = New;
    start = New;

    temp->next = start;

    Display_Linked_List(start);
    return start;
}

void Insert_From_end(Node* start)
{
    int elem;
    Node* temp = start;
    cout<<endl<<"enter element to insert at the end -> ";
    cin>>elem;
    Node* New = new Node(elem);
    New->next = NULL;
    New->prev = NULL;
    Display_Linked_List(start);

    do
    {
        temp=temp->next;
    }while(temp->next != start);
    temp->next = New;
    New->prev = temp;
    temp = New;
    temp->next = start;
    Display_Linked_List(start);
}

Node* Delete_From_start(Node* start)
{
    if(start == NULL)
    {
        cout<<endl<<"Linked List is empty ";
        return NULL;
    }

    Display_Linked_List(start);

    Node* temp;
    Node* rstart = start;
    temp = start;
    cout<<endl<<start->data<<" is deleted successfully ";

    do
    {
        temp=temp->next;
    }while(temp->next != start);
    temp->next = start->next;
    start = start->next;
    start->prev = NULL;
    rstart->next = NULL;
    rstart->prev = NULL;
    delete rstart;
    
    Display_Linked_List(start);
    

    return start;
}

void Delete_From_end(Node* start)
{
    if(start == NULL)
    {
        cout<<endl<<"Linked List is empty ";
        return;
    }
    Display_Linked_List(start);

    Node* temp;
    temp = start;
    do
    {
        temp=temp->next;
    }while(temp->next != start);
    temp->prev->next = start;
    temp->next = NULL;
    temp->prev = NULL;
    cout<<endl<<temp->data<<" is deleted successfully";
    delete temp;

    Display_Linked_List(start);

}

void Delete_Specific_Elem(Node* start)
{
    Node* rstart = start;
    cout<<endl<<"enter element to delete ";
    int elem;
    cin>>elem;
    int counter = 0;
    Node* temp = start;
    while(temp->next != start && temp->data != elem)
    {
        counter++;
        temp=temp->next;
    }

    if(temp == start)
    {
        Display_Linked_List(start);
        Node* temp;
        Node* rstart = start;
        temp = start;
        cout<<endl<<start->data<<" is deleted successfully ";

        do
        {
            temp=temp->next;
        }while(temp->next != start);
        temp->next = start->next;
        start = start->next;
        start->prev = NULL;
        rstart->next = NULL;
        rstart->prev = NULL;
        delete rstart;
        Display_Linked_List(start);
    }
    else if(temp->next == start)
    {
        Display_Linked_List(start);
        temp->prev->next = start;
        temp->next = NULL;
        temp->prev = NULL;
        cout<<endl<<temp->data<<" is deleted successfully";
        delete temp;
        Display_Linked_List(start);
    }
    else if(temp->next != start)
    {
        Display_Linked_List(start);
        cout<<endl<<elem<<" found at "<<counter+1;
        temp->prev->next = temp->next;
        temp->next->prev = temp->prev;
        temp->next = NULL;
        temp->prev = NULL;
        delete temp;
        cout<<endl<<elem<<" is deleted successfully ";
        Display_Linked_List(start);
    }
    else
    {
        cout<<endl<<" Element Not Found ";
    }
}

void Delete_Kth_Elem(Node* start)
{
    Node* rstart = start;
    cout<<endl<<"enter element number to be delete ";
    int n;
    cin>>n;
    Node* temp = start;

    for(int i=1;i<n;i++)
    {
        temp = temp->next;
    }
    if(temp != NULL)
    {
        cout<<endl<<temp->data<<" is deleted successfully";
    }
    else
    {
        cout<<endl<<" Linked List is empty ";
    }

    if(temp == start)
    {
        Display_Linked_List(start);
        Node* temp;
        Node* rstart = start;
        temp = start;

        do
        {
            temp=temp->next;
        }while(temp->next != start);
        temp->next = start->next;
        start = start->next;
        start->prev = NULL;
        rstart->next = NULL;
        rstart->prev = NULL;
        delete rstart;
        Display_Linked_List(start);
    }
    else if(temp->next == start)
    {
        Display_Linked_List(start);
        temp->prev->next = start;
        temp->next = NULL;
        temp->prev = NULL;
        delete temp;
        Display_Linked_List(start);
    }
    else if(temp->next != start)
    {
        Display_Linked_List(start);
        temp->prev->next = temp->next;
        temp->next->prev = temp->prev;
        temp->next = NULL;
        temp->prev = NULL;
        delete temp;
        Display_Linked_List(start);
    }
    else
    {
        cout<<endl<<" Element Not Found ";
    }
}



Node* Insert_Specific_Elem(Node* start)
{
    Display_Linked_List(start);
    cout<<endl<<"enter element after which you want to insert ->";
    int elem;
    cin>>elem;
    
    cout<<endl<<"enter what to insert ";
    int val;
    cin>>val;

    Node* temp = start;
    
    while(temp->next != start && temp->data != elem)
    {
        temp=temp->next;
    }

    if(temp == NULL)
    {
        Node* New=new Node;
        New->data = val;
        New->prev = NULL;
        New->next = start;
        start = New;
    }
    else if(temp->next == start)
    {
        Node* New = new Node;
        New->data = val;
        New->next = start;
        New->prev = temp;
        temp->next = New;
    }
    else
    {
        Node* New = new Node;
        New->data = val;
        New->next = temp->next;
        New->prev = temp;
        temp->next->prev = New;
        temp->next = New;
    }
    Display_Linked_List(start);
    return start;
}

void Searching_Linked_List(Node* start)
{
    Node* temp = start;
    cout<<endl<<"enter the element to be seached-> ";
    int elem;
    cin>>elem;
    int counter = 0;
    do
    {
        counter++;
        temp=temp->next;
    }while(temp->next != start && temp->data != elem);

    if(temp != NULL)
    {
        cout<<endl<<elem<<" is found at "<<counter+1;
    }
    else
    {
        cout<<endl<<elem<<" not found ";
    }
}

void Middle_linked_list_naive(Node* start)
{
    Node* temp = start;
    int counter = 0;
    do
    {
        counter++;
        temp = temp->next;
    }while(temp->next != start);
    temp = start;
    for(int i=0;i<counter/2;i++)
    {
        temp = temp->next;
    }
    if(counter%2 != 0)
    {
        cout<<endl<<temp->data<<" & "<<temp->next->data;
        cout<<" is at middle of linked list";
    }
    else
    {
        cout<<endl<<temp->data <<" is at middle of linked list";
    }
}

void Middle_linked_list_efficient(Node* start)
{
    Node* temp = start;
    Node* slow = start;
    Node* fast = start;
    slow = slow->next;        
    fast = fast->next->next;        

    while(fast->next != start && fast != start)
    {
        slow = slow->next;        
        fast = fast->next->next;        
    }
    // for linked list
    // }while(fast->next != NULL || fast != NULL);

    if(fast == start)
    {
        cout<<endl<<slow->prev->data<<" & "<<slow->data;
        cout<<" is at middle of linked list";
    }
    else
    {
        cout<<endl<<slow->data<<" is at middle of linked list";
    }
}

void Nth_Node_from_end_naive(Node* start)
{
    cout<<endl<<"Enter K value from the end "<<endl;
    int k;
    cin>>k; 

    int counter = 0;
    Node* temp = start;
    do
    {
        counter++;
        temp = temp->next;
    }while(temp->next != start);

    temp = start;
    for(int i=0;i<counter-k+1;i++)
    {
        temp = temp->next;
    }
    cout<<endl<<temp->data<<" is present at ";
    cout<<k<<" node from th end";
}

void Nth_Node_from_end_efficient(Node* start)
{
    cout<<endl<<"Enter K value from the end "<<endl;
    int k;
    cin>>k; 

    Node* temp = start;
    Node* slow = start;
    Node* fast = start;

    for(int i=1;i<k;i++)
    {
        if(slow->next == start) 
        {
            return;
        }
        slow = slow->next;
    }        

    while(slow->next != start)
    {
        slow = slow->next;
        fast = fast->next;
    }
    cout<<endl<<fast->data<<" is present at ";
    cout<<k<<" node from th end";
}

void Reverse_linked_list_naive(Node* start)
{
    Display_Linked_List(start);
    vector <int> arr;
    auto it  = start;
    arr.push_back(it->data);
    it = it->next;
    while(it != start)
    {
        arr.push_back(it->data);
        it = it->next;
    }

    it  = start;
    it->data = arr.back();
    arr.pop_back();
    it = it->next;
    while(it != start)
    {
        it->data = arr.back();
        arr.pop_back();
        it = it->next;
    }
    Display_Linked_List(start);
}

Node* Reverse_linked_list_efficient(Node* start)
{
    Display_Linked_List(start);
    Node* temp;
    Node* prev;
    Node* curr;
    temp = start;
    prev = temp;
    temp = temp->next;
    prev->next = NULL;
    while(temp != start)
    {   
        curr = temp;
        temp = temp->next;
        curr->next = prev;
        prev = curr;
    }
    start = prev;
    Display_Linked_List(start);
    return start;
}

int main()
{
    Node* start = new Node();
    int n;
    do
    {     
        cout<<endl<<"1-> Create a Linked List ";
        cout<<endl<<"2-> Insert Element at the start ";
        cout<<endl<<"3-> Insert Element at the end";
        cout<<endl<<"4-> Insert at specific element";
        cout<<endl<<"5-> Delete Element at the start ";
        cout<<endl<<"6-> Delete Element at the end";
        cout<<endl<<"7-> Delete at specific element";
        cout<<endl<<"8-> Delete Kth element in a linked list";
        cout<<endl<<"9-> Transverse all element ";
        cout<<endl<<"10-> Search a Particular Element ";
        cout<<endl<<"11-> Middle of Linked List (naive) ";
        cout<<endl<<"12-> Middle of Linked List (efficient) ";
        cout<<endl<<"13-> Nth Node from end (naive) ";
        cout<<endl<<"14-> Nth Node from end (efficient) ";
        cout<<endl<<"15-> Reverse A linked List (naive) ";
        cout<<endl<<"16-> Reverse A linked List (efficient) ";
        cout<<endl<<"17-> Exit  ";
        cout<<endl<<" Please Enter your Choice ";
        cin>>n;
        switch(n)
        {
            case 1:
                start = Create_Linked_List();
            break;

            case 2:
                start = Insert_From_start(start);
            break;

            case 3:
                Insert_From_end(start);
            break;

            case 4:
                start = Insert_Specific_Elem(start);
            break;

            case 5:
                start = Delete_From_start(start);
            break;

            case 6:
                Delete_From_end(start);
            break;

            case 7:
                Delete_Specific_Elem(start);
            break;

            case 8:
                Delete_Kth_Elem(start);
            break;

            case 9:
                Display_Linked_List(start);
            break;

            case 10:
                Searching_Linked_List(start);
            break;

            case 11:
                Middle_linked_list_naive(start);
                //  It is taking two loop
            break;

            case 12:
                Middle_linked_list_efficient(start);
                //  It is taking one loop
                // so done in one transversal
            break;

            case 13:
                Nth_Node_from_end_naive(start);
            break;

            case 14:
                Nth_Node_from_end_efficient(start);
            break;

            case 15:
                Reverse_linked_list_naive(start);
            break;

            case 16:
                start = Reverse_linked_list_efficient(start);
            break;

            case 17:
                exit(0);
            break;

            default: 
                cout<<endl<<"wrong choice plz try again";
        }
    }while(n != 10);

    return 0;
}
